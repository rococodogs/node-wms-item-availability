...
function ItemAvailability (wskey, aii) {
  if (!(this instanceof ItemAvailability)) return new ItemAvailability(wskey, aii)

  this.wskey = wskey
  this.aii = aii || (this.wskey.user ? this.wskey.user.institutionId : null)

  if (!this.wskey) throw Error('No WSKey provided')

  if (!this.aii) throw Error('ItemAvailability requires an authenticatingInstitutionId')
}
